    ...
    return (
        ...
        {props.isTransitioning && (
            <div className="fixed top-1/2 left-1/2 z-[210] pointer-events-none" style={{ animation: 'pow-enter 1s forwards ease-out' }}>
                <svg viewBox="0 0 200 150" className="w-[500px] h-[400px] drop-shadow-[0_10px_0_rgba(0,0,0,0.5)]">
                    ...
                    <text x="100" y="95" textAnchor="middle" fontFamily="'Russo One', sans-serif" fontSize="70" fill="#DC2626" stroke="black" strokeWidth="2" transform="rotate(-5 100 75)">POW!</text>
                </svg>
            </div>
        )}
        ...
        <div className={`fixed inset-0 z-[200] overflow-y-auto`}
          ...
          <div className="min-h-full flex items-center justify-center p-4 pb-8">
            ...
            <div className="max-w-[900px] w-full bg-white p-4 md:p-5 rotate-1 border-[6px] border-black shadow-[12px_12px_0_rgba(0,0,0,0.6)] text-center relative">
                
                <h1 className="font-comic text-4xl md:text-5xl text-red-600 leading-none mb-1 tracking-wide inline-block mr-3" style={{textShadow: '2px 2px 0px black'}}>БЕСКОНЕЧНЫЕ</h1>
                <h1 className="font-comic text-4xl md:text-5xl text-yellow-400 leading-none mb-4 tracking-wide inline-block" style={{textShadow: '2px 2px 0px black'}}>ГЕРОИ</h1>
                
                <div className="flex flex-col md:flex-row gap-4 mb-4 text-left">
                    
                    {/* Left Column: Cast */}
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-black border-b-4 border-black mb-1">1. ГЕРОИ</div>
                        ...
                        <div className={`p-3 border-4 border-dashed ${props.hero ? 'border-green-500 bg-green-50' : 'border-blue-300 bg-blue-50'} transition-colors relative group`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-bold text-lg uppercase text-blue-900">ГЛАВНЫЙ ГЕРОЙ (ОБЯЗАТЕЛЬНО)</p>
                                {props.hero && <span className="text-green-600 font-bold text-sm animate-pulse">✓ ГОТОВО</span>}
                            </div>
                        ...
                        <div className={`p-3 border-4 border-dashed ${props.friend ? 'border-green-500 bg-green-50' : 'border-purple-300 bg-purple-50'} transition-colors`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-bold text-lg uppercase text-purple-900">НАПАРНИК (ОПЦИОНАЛЬНО)</p>
                                {props.friend && <span className="text-green-600 font-bold text-sm animate-pulse">✓ ГОТОВО</span>}
                            </div>
                        ...
                        <p className="text-[10px] text-gray-500 leading-tight mt-1 px-1">
                            Применяется Политика запрещенного использования. Не создавайте контент, нарушающий права конфиденциальности других лиц.
                        </p>
                    </div>

                    {/* Right Column: Settings */}
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-black border-b-4 border-black mb-1">2. СЮЖЕТ</div>
                        
                        <div className="bg-yellow-50 p-3 border-4 border-black h-full flex flex-col justify-between">
                            <div>
                                <div className="mb-2">
                                    <p className="text-base mb-1 font-bold text-gray-800">ЖАНР</p>
                                    <select value={props.selectedGenre} onChange={(e) => props.onGenreChange(e.target.value)} className="w-full font-bold text-lg p-1 border-2 border-black uppercase bg-white text-black cursor-pointer shadow-[3px_3px_0px_rgba(0,0,0,0.2)] focus:outline-none focus:translate-x-[1px] focus:translate-y-[1px] focus:shadow-none transition-all">
                                        {GENRES.map(g => <option key={g} value={g} className="text-black">{g}</option>)}
                                    </select>
                                </div>

                                <div className="mb-2">
                                    <p className="text-base mb-1 font-bold text-gray-800">ЯЗЫК</p>
                                    <select value={props.selectedLanguage} onChange={(e) => props.onLanguageChange(e.target.value)} className="w-full font-bold text-lg p-1 border-2 border-black uppercase bg-white text-black cursor-pointer shadow-[3px_3px_0px_rgba(0,0,0,0.2)]">
                                        {LANGUAGES.map(l => <option key={l.code} value={l.code} className="text-black">{l.name}</option>)}
                                    </select>
                                </div>

                                {props.selectedGenre === 'Свой сюжет' && (
                                    <div className="mb-2">
                                        <p className="text-base mb-1 font-bold text-gray-800">ЗАВЯЗКА</p>
                                        <textarea value={props.customPremise} onChange={(e) => props.onPremiseChange(e.target.value)} placeholder="Опишите завязку сюжета..." className="w-full p-1 border-2 border-black font-bold text-lg h-16 resize-none shadow-[3px_3px_0px_rgba(0,0,0,0.2)]" />
                                    </div>
                                )}
                            </div>
                            
                            <label className="flex items-center gap-2 text-base cursor-pointer text-black mt-1 p-1 hover:bg-yellow-100 rounded border-2 border-transparent hover:border-yellow-300 transition-colors font-bold">
                                ...
                                <span className="text-black">РЕЖИМ НОВЕЛЛЫ (Больше текста)</span>
                            </label>
                        </div>
                    </div>
                </div>

                <button onClick={props.onLaunch} disabled={!props.hero || props.isTransitioning} className="comic-btn bg-red-600 text-white text-2xl md:text-3xl px-6 py-3 w-full hover:bg-red-500 disabled:bg-gray-400 disabled:cursor-not-allowed uppercase tracking-wider">
                    {props.isTransitioning ? 'ЗАПУСК...' : 'НАЧАТЬ ПРИКЛЮЧЕНИЕ!'}
                </button>
            </div>
          </div>
        </div>
